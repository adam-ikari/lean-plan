...
- 保住缓存前缀。一个任务一个会话、工具面固定、少派 worker，缓存前缀不被打断，后面的请求主要为增量付费。实测命中率：干净单会话跑到 73%，有返工的落到 30%。
...
- 缓存命中率遵循同一套规则。干净的单会话批量跑到 73% 缓存读取，多 worker 或有返工的落到 30% 到 57%。少开新会话、中途不动工具、少派 worker、不重复读文件，都在保住前缀。
